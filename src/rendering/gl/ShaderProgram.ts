...
class ShaderProgram {
  ...
  setCamerapos(time: vec4) {
    this.use();
    if (this.unifCameraPos !== -1) {
      gl.uniform4fv(this.unifCameraPos, time);
    }
  }

  setMaxRoof(time: number) {
    this.use();
    if (this.unifMaxRoof !== -1) {
      gl.uniform1f(this.unifMaxRoof, time);
    }
  }

  setMinRoof(time: number) {
    this.use();
